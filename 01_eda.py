...
# "DAT109 00:00" 같은 time 문자열을 timedelta로 변환하는 함수
...
# → (2026-07-10 반영) 확정된 로직은 preprocess.py로 이미 분리 완료.


# =========================================
# 2. 구동기(액추에이터) 컬럼 분류 — train/test 값 비교
# =========================================
actuator_cols = [
...
# test_X에서도 같은 구동기 컬럼들이 어떤 값을 갖는지 확인 (train과 비교)
print("\n=== test_X 구동기 컬럼 값 확인 ===")
for col in actuator_cols:
...
#   즉 test 구간에서 천창1이 train에서 한 번도 본 적 없는 31~59 사이의 값으로 열린 적이 있다는 뜻입니다.
#   모델은 train으로 학습하기 때문에 이 범위는 "본 적 없는 입력"이 되고, 트리 기반 모델이면 그 범위를
#   잘 다루지 못하고 근처 학습값으로 뭉개버릴 수 있고, min-max 스케일링 계열이면 범위 밖 값이라 스케일이
#   깨질 수 있습니다. → 이건 이상치라기보다 "train이 test의 실제 동작 범위를 다 커버하지 못한다"는
#   구조적 리스크로 봐야 합니다.
#
...
# =========================================
# 3. 날씨/내부환경 컬럼 train vs test 범위 비교
# =========================================
env_cols = [
...
# temperature(내부 온도): train min 1.50 vs test min -0.20 → 내부 온도가 train에서 한 번도 본 적 없는 영하까지
#   떨어짐. 이건 신경 써야 합니다 — 이 컬럼이 목표값(soil_temp)과 직접 관련이 클 가능성이 높은데, 모델이
#   학습 못 한 저온 구간에서 예측을 해야 하는 상황이라 오차가 커질 수 있습니다.
...
# 이 세 컬럼은 나중에 모델링할 때 클리핑하거나, 스케일링 기준을 train만이 아니라 train+test 전체 범위로 잡는
# 등 대비가 필요합니다.
...
#   습도 센서가 물리적으로 100%(포화)에 닿은 건데, 진짜 100%일 수도 있지만 센서가 100%에서 막혀서(clip)
#   그 이상 값을 못 찍은 걸 수도 있습니다. 나중에 이 값이 얼마나 자주 100으로 찍히는지(연속으로 붙어있는지)
#   확인해볼 가치는 있습니다.
#
# 2) soil_ec(train_y) — 오른쪽으로 크게 치우친 분포
#   평균 0.91, 표준편차 0.55인데 max가 2.92 → 평균에서 표준편차 3.6배 넘게 떨어진 값입니다. 25%(0.505)~75%(1.617)
#   사이 폭도 크고요. 물리적으로 틀린 값은 아니지만, 다음 단계(IQR 이상치 확인)에서 이 컬럼은 정상적으로 꽤
#   많은 값이 "통계적 이상치"로 잡힐 가능성이 높습니다 → 물리적 이상치가 아니라 원래 그런 분포라는 걸
#   구분해야 합니다.
#
# 3) solar_radiation — 평균(79.7)보다 표준편차(132.8)가 더 큼
#   낮/밤 주기 때문에 0인 값이 절반 가까이 되고 낮에만 값이 튀는 구조라 원래 이렇게 치우칩니다. 이 컬럼에
#   IQR을 그대로 적용하면 "맑은 날 정오"처럼 정상적인 고일사량 시간대가 전부 이상치로 잘못 잡힐 수 있어서
#   주의가 필요합니다.
#
...
# soil_temp 352개(4.7%): 26일 동안 계절/날짜에 따라 배지 온도가 서서히 변한 걸로 보임 — 하루 중 변화라기보다
#   기간에 걸친 자연스러운 추세.
# → 이 5개 컬럼은 "이상치 제거" 하면 안 됩니다. 실제 신호를 5~13%씩 날려버리는 꼴이라 오히려 모델 성능을
#   해칩니다.
#
# 중요한 검증 — soil_ec 이상치 0개
#   지난번 "오른쪽으로 치우친 분포라 이상치 많이 잡힐 것"이라 예상했는데, 결과는 정반대로 0개입니다. IQR이
#   Q1~Q3 폭(1.11) 자체를 넓게 잡아서 max(2.92)까지 정상범위(~3.28) 안에 다 들어옵니다. 즉 skew가 있어도
#   "일관되게 치우친 분포"라 IQR 계산 자체엔 문제 없었다는 뜻 — 지난번 우려가 실제로는 기우였다는 게
#   확인됐습니다.
#
# 진짜 눈여겨볼 것 — 개수가 적어서 직접 확인 가능한 것들
#   soil_moisture 이상치 4개: 7,488개 중 4개면 개별로 뜯어봐도 되는 수준. 급격한 관수(irrigation) 이벤트인지
#     센서 튐인지 확인 가치 있음.
#   내부 습도 100.0 — test에서 523번 (전체 17,280분의 약 3%). train은 0번인데 test(겨울)에서만 나타남.
#     결로/포화로 인한 진짜 100%인지, 연속으로 오래 붙어있어서 센서가 막힌 건지 패턴을 봐야 함.


# =========================================
# 7. soil_moisture 이상치 4개 직접 확인
# =========================================
...
# =========================================
# 8. 내부 습도 100% 연속 구간(run) 확인 — 센서 고착 여부
# =========================================
...
# soil_moisture 이상치 4개는 DAT128 저녁 한 시간 반 안에 뭉쳐있고, soil_ec 급등/온도 상승 동반 →
#   시비/관수 이벤트로 추정 (자세한 수치는 README.md 참고)
# 내부 습도 100% 연속 구간은 test에서 7번, 최대 182분 지속 → 야간 결로로 추정, 센서 오류 아님
#
# (2026-07-10) dt 인덱스 활성화 완료, 확정 전처리 로직은 preprocess.py로 분리 완료.
# 다분광(ms) 데이터 EDA는 02_ms_eda.py, 전체 진행상황은 PLAN.md, 핵심 인사이트 모음은 README.md 참고.
