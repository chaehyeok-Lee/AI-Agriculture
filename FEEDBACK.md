...
# 셀프 피드백 루프 11 — 26.07.14 (soil_moisture, soil_temp급 정밀 튜닝 시도 — 사용자 요청, 중단 지시까지 계속)

**동기**: soil_temp에 적용했던 것과 같은 수준의 정밀 튜닝(정규화 그리드서치, 신규 피처)을
soil_moisture에도 적용해달라는 요청. 현재 production 기준 soil_moisture 4-fold **1.0541**.

## 라운드 1: LightGBM 정규화 그리드서치 (num_leaves/feature_fraction/reg_lambda/min_child_samples/bagging_fraction)

**방법**: soil_temp가 num_leaves=8·feature_fraction=0.8·reg_lambda=3.0·min_child_samples=5로
큰 개선(-42%)을 봤던 것과 동일한 축을 순차 탐색(한 축씩 최적값 고정 후 다음 축).

**결과**: **5개 축 전부, 모든 값에서 현재 기본값(num_leaves=31·ff=1.0·reg_lambda=0·
min_child=20·bagging=1.0)이 이미 최적** — 어느 방향으로 바꿔도 악화(num_leaves 7/15/63
전부 +3~4%, feature_fraction 낮출수록 최대 +23%까지 급격 악화, reg_lambda 올릴수록 +2~8%,
min_child_samples 올릴수록 +7%, bagging_fraction 전부 +3~9%).

**판단**: soil_temp와 정반대 결론. soil_temp는 완만한 일교차 곡선이라 정규화(스무딩)가
도움됐지만, soil_moisture는 115/121/129일의 **뚜렷한 계단형 전환**이 핵심 신호라 모델이
전체 분기 능력을 다 써야 그 계단을 잡아낼 수 있음 — 정규화로 트리를 단순화시키면 계단을
뭉개버려 오히려 악화. **폐기, 코드 변경 없음** (현재 기본 파라미터가 이미 이 데이터에
최적임을 재확인).

## 라운드 2: 신규 피처 4종 (baseline에 단독 추가)

| 피처 | 결과 | 판단 |
|---|---|---|
| `ec_high_confidence` (EC 관리이벤트 신뢰도, 원래 EC 전용) | 무변화(+0.0000, 0-importance) | 폐기 — moisture 전환과는 무관한 신호 |
| `humidity_mean_roll1152`(4일 rolling) | **+11.5% 악화** | 폐기 — 너무 긴 창이 최근 레벨 변화를 뭉갬 |
| `humidity_mean_trend2d`/`circ_fan_mean_trend2d`(2일 변화량) | +1.9% 악화 | 폐기 |
| `day_num × humidity_mean`(상호작용) | +1.5% 악화 | 폐기 |

**판단**: 4개 전부 기각. `ec_high_confidence`가 정확히 0-importance로 전혀 안 쓰인 건
day_num이 이미 시간축 정보를 다 갖고 있어서 추가 신호가 불필요하다는 뜻으로 해석됨.

## 라운드 3: 레짐 지속시간(regime duration) 계열 피처 3종

| 피처 | 결과 | 판단 |
|---|---|---|
| `circ_fan_regime_duration`(레짐 시작 후 경과시간) | +2.4% 악화 | 폐기 |
| `humidity_mean_accel12h`(12h-1d 가속도) | +1.1% 악화 | 폐기 |
| `vent1_on_binary`+`vent1_regime_duration` | +2.6% 악화 | 폐기 |

**판단**: 9개 아이디어(라운드1 정규화 5축 + 라운드2 신규피처 4종 + 라운드3 레짐지속 3종
= 총 12개 시도) 전부 실패. day_num이 이미 "경과시간"이라는 정보를 압축적으로 담고 있어서
파생 시간 피처를 추가해도 중복 정보만 될 뿐 새 신호가 안 됨 — 계단 전환 자체(115/121/129일)는
여전히 X변수만으로 선행 예측 불가능한 관리 이벤트라는 결론(FEEDBACK.md 상단 기록)과 일치.

## 라운드 4: 부스팅 알고리즘/lr/멀티시드

| 시도 | 결과 |
|---|---|
| boosting_type=goss | +4.4% 악화 |
| boosting_type=dart | **+46.2% 대폭 악화** (dart는 early stopping 미지원이라 n_est=1000 그대로 다 써서 과적합) |
| learning_rate=0.02/0.08/0.1 | 전부 +1.2~2.1% 악화 |
| 멀티시드 3개/5개 앙상블 | **완전 동일**(delta=0) — early stopping이 seed 무관하게 같은 수렴점 도달 (EC/temp에서 이미 확인된 패턴과 동일) |

**누적**: 4라운드 15개 아이디어 전부 실패. 현재 기본 설정이 이 데이터에서 진짜 지역 최적점.

## 라운드 5: 커튼 불일치 / day_num 단조제약 / huber loss

| 시도 | 결과 |
|---|---|
| `curtain_disagree`(thermal_curtain-shading_curtain 절대차) | +0.7% 소폭 악화 |
| day_num monotone_constraints(+1, 단조증가 강제) | **+7.2% 악화** — day_num-moisture 관계가 실제로 비단조(계단형 오르내림)라 강제하면 손해 |
| objective=huber | +7.7% 악화 |

**누적 18개 아이디어 전부 실패**(라운드1~5). 이 시점에서 구조적 한계 가능성이 매우 높지만
사용자 지시(중단 전까지 계속)에 따라 다른 범주(코드 잠재 버그, 타깃 변환, 이미지 재시도) 계속 시도.

## 라운드 6: 코드 잠재이슈(dead code) + 타깃 로그변환

**발견**: `MOISTURE_LAG_EXCLUDE`(humidity_mean lag만 허용하는 목록)가 정의만 되고
`DROP_COLS_PER_TARGET["soil_moisture"]`에서 실제로는 안 쓰이고 있었음(지금은
`LAG_FEATURE_NAMES` 전체가 무조건 drop됨) — 방치된 코드라 실제로 적용해보면 다를지 확인.

| 시도 | 결과 |
|---|---|
| humidity_mean lag 6개만 허용(dead code 실제 적용) | +0.5% 소폭 악화 |
| moisture 로그변환 학습 → exp 복원 | +6.9% 악화 |

**판단**: dead code는 죽어있는 게 맞았음(활성화해도 손해) — 폐기, 코드 정리 불필요(어차피
`DROP_COLS_PER_TARGET`가 최종 소스이므로 `MOISTURE_LAG_EXCLUDE` 변수 자체는 무해하게 방치).

**누적 20개 아이디어 전부 실패**(라운드1~6: 정규화 5축, 신규피처 4종, 레짐지속 3종,
부스팅/lr/멀티시드 5종, 커튼불일치/단조제약/huber 3종, dead-code lag/로그변환 2종).

## 라운드 7: 다분광 위치0(넓은 고정카메라) 10밴드 평균 재검증

**배경**: 이전 세션(파이프라인 개편 전)에서 위치0 raw band 단독 추가가 3개 타깃 중
유일하게 악화 없이 소폭 개선(moisture 1.1689→1.1529)됐다는 기록(PLAN.md) — 그 뒤
피처 파이프라인이 크게 바뀌어서 현재 기준 재검증.

**결과**: train 유효 일수 7/26(결측 73.1%), 4-fold **+1.6% 악화**로 이번엔 개선 없음.

**판단**: 결측률이 워낙 높고(관측 자체가 희소) 신호도 약해서, 지금 피처 조합에서는
오히려 노이즈로 작용. 폐기 확정 — 다분광 카메라 파장 한계(713~920nm, moisture 수분
스트레스엔 970nm+ 필요)라는 기존 결론과 일치.

**누적 21개 아이디어 전부 실패**.

## 라운드 8: HistGradientBoostingRegressor 단독 (대안 모델 유형)

**결과**: LightGBM 1.0541 대비 HGBT 1.1976, **+14.4% 대폭 악화**(특히 fold1 1.80으로 폭발).

**판단**: 기존 EC/temp에서 확인된 "HGBT 단독 열세" 패턴이 moisture에도 동일하게 적용됨.
폐기 확정. **누적 22개 아이디어 전부 실패**.

## 8라운드 종합 (사용자 지시로 중단 시까지 계속 진행 중)

| 범주 | 시도 수 | 결과 |
|---|---|---|
| LightGBM 정규화(num_leaves/feature_fraction/reg_lambda/min_child/bagging) | 5축 | 전부 악화 |
| 신규 피처(ec_high_confidence/rolling4d/trend2d/상호작용) | 4 | 전부 무변화~악화 |
| 레짐 지속시간 계열 | 3 | 전부 악화 |
| 부스팅 알고리즘/lr/멀티시드 | 5 | 전부 악화(dart는 +46%) |
| 커튼불일치/단조제약/huber loss | 3 | 전부 악화 |
| dead code 활성화/타깃 로그변환 | 2 | 전부 악화 |
| 다분광 위치0 이미지 재검증 | 1 | 악화(결측 73%) |
| 대안 모델(HistGradientBoosting) | 1 | 대폭 악화(+14.4%) |
| **합계** | **22개** | **22개 전부 실패, 0승** |

**결론**: soil_ec의 "0.2 목표 도달 불가"(루프4)와 같은 성격의 구조적 한계로 판단됨 —
현재 feature 조합 + LightGBM 기본 설정이 이미 이 데이터의 지역 최적점이며, day_num이
시간축 정보를 이미 압축적으로 담고 있어 파생 시간/레짐 피처가 전부 중복 정보로 전락함.
115/121/129일의 계단형 전환은 X변수로 선행탐지 불가능한 관리 이벤트. **코드 변경 없음,
4-fold mean 1.0541 유지.**

---

# 셀프 피드백 루프 11 — 26.07.14 (soil_temp fold4 집중 규명 — 30여 개 시도 전부 실패)

**동기**: soil_temp 4-fold(0.5583)에서 fold4(cutoff=130, val=130~133일)만 유독 나쁨
(0.8422, 나머지 3개는 0.35~0.53). 상관계수(r=0.976)·진폭(기울기=0.975)은 오히려 다른
fold보다 좋은데 RMSE만 압도적으로 나쁜 게 이상해서, "왜 나쁜가"부터 "고칠 수 있는가"까지
철저히 파봄.

## 1단계: 원인 진단 — 편향(bias) 문제로 확정

fold4만 편향 +0.667(다른 fold는 -0.15~+0.05) — 상관계수/진폭은 정상인데 **체계적으로
과대예측**. LightGBM 단독(+0.69)과 Ridge 단독(+0.62) 둘 다 비슷하게 편향돼 있어 블렌드
비율 문제가 아님(오히려 블렌드가 둘 중 어느 단독보다 나음).

## 2단계: 근본 원인 규명 — "같은 외부신호, 다른 반응 크기"

fold4 검증구간(130~133일) 중 133일이 압도적 문제아. 훈련구간(109~129일) 안에 **같은
날씨 템플릿이 이미 3번(113/119/125일) 있었음**(외기온도 하락 패턴이 4일 모두 3시간
단위로 소수점까지 완전 동일 — 날씨 데이터 복제 구조가 여기도 적용됨). 그런데 실제
soil_temp 하락폭이 113일(2.79도)/119일(2.49도)/125일(2.37도)는 서로 비슷한데 **133일만
3.83도**로 튐(이전 3번의 자체 편차 0.43도보다 2배 넘게 큰 이탈).

**farmer 시나리오 조사**: 액추에이터(농부가 직접 조작하는 커튼/팬)를 비교하니 133일만
확연히 다름 — 다른 3일은 정오에 보온커튼을 100%까지 다 여는데 **133일은 최대 63%까지만
열림**, 순환팬은 다른 3일(119/125일=거의 꺼짐) 대비 **133일=하루 종일 70~100% 가동**.
가설: 온도가 예상보다 빠르게 떨어지자 순환팬을 더 돌리고 커튼을 덜 열어 대응했지만(반응),
그 과정에서 낮 동안의 자연 태양열 획득 기회를 놓쳐 결과적으로 하루 평균이 더 떨어짐.
이 반응 자체가 133일 단 1회뿐이라 통계적으로 학습 불가능한 표본.

## 3단계: 해결 시도 전부 실패 (범주별 정리)

| 범주 | 시도 | 결과 |
|---|---|---|
| 단기 추세 피처 | temperature_mean/humidity_mean trend 1d/6h/3h, 조합 | 5개 전부 악화(+3~5%) |
| 한파 이탈도 피처 | cold_snap_dev(외기온도 3일 baseline 대비), ×lag12 상호작용 | 3개 전부 악화(+4.8%), fold4도 무변화 |
| 조건부 가중치 | 한파 유사 샘플 3배 가중 | 악화(+9.0%) |
| 최근일 가중치(recency) | half_life 1~30일 스윕(9개 값) | half_life=1.5일이 전체는 -1.3%로 유일한 순개선이나, fold1~3 전부 10%씩 악화(재분배일 뿐) + 최종모델 적용시 학습데이터 90%를 마지막 5일에 집중시키는 과도한 위험 → **미채택** |
| 액추에이터 이례성 피처 | circ_fan/커튼의 "같은 시간대 최근일 대비 이탈도" v1(버그, 당일내 스무딩) + v2(제대로 된 일단위 비교) | 둘 다 악화(v2는 fold4가 오히려 0.93으로 더 나빠짐) |
| 대안 모델(전체 피처) | RandomForest/ExtraTrees/HistGB/KNN(k=5~15) | 4종 전부 현재보다 30~170% 나쁨 |
| 대안 모델(피처 큐레이션) | KNN 10개 축소피처, RF/ET importance top20 | 개선은 되나(KNN -44%, ET -12%) 여전히 현재보다 18%+ 나쁨 |
| 최선 대안모델 블렌드 | ExtraTrees(top20) 가중치 5~50% | 가중치 0%(안 섞음)가 최선, 섞을수록 단조 악화 |
| LightGBM 정규화 | path_smooth 0.1~5.0(5개 값) | 전부 악화, fold4만 미세 개선(대가 더 큼) |
| 손실함수 | huber, fair | 둘 다 대폭 악화(+6.3%, +7.5%) |

**누적 30개 이상 시도, 전부 실패 또는 재분배(한쪽 fold 개선 시 다른 쪽 악화, 순이익 없음).**

## 최종 결론

fold4의 오차는 피처 설계나 모델 선택의 문제가 아니라 **"관측되지만 원인 재구성이 안 되는
1회성 관리 반응"**이 원인 — soil_ec의 관리 이벤트(115/121/129일) 문제와 동일한 성격의
정보 부재 한계. 액추에이터 값 자체는 이미 모델이 매 시점 보고 있지만(1) 반응이 감지되는
시점(9시 이후)이 이미 하루 손실 상당분이 누적된 뒤이고(2) 표본이 1개뿐이라 트리가 일반
규칙으로 학습할 근거가 없음. **train.py 코드 변경 없음, 4-fold soil_temp mean 0.5583
유지.** 추가 개선하려면 배지 함수율의 온도모델 입력 활용, 난방 설정온도 로그 등 현재
20개 X컬럼 밖의 새 데이터가 필요.
