...
## 최종 성능 (26.07.14) — 3개 타깃 전부 구조적 한계 도달 확정

| 타깃 | 기준선 4-fold | 최종 4-fold | 개선율 | 비고 |
|---|---|---|---|---|
| soil_moisture | 1.1199 | **1.0541** | -5.9% | rolling/accel 피처, 22개 추가시도 전부 실패(8라운드) |
| soil_ec | 0.3106 | **0.2899** | -6.7% | 소프트 블렌드(루프9) + 다분광 최소반영(루프10) |
| soil_temp | 1.0771 | **0.5583** | **-48.2%** | lag+Ridge+하이퍼파라미터, fold4 원인규명+30여개 시도 전부 실패(루프11) |
...
## soil_temp 구조적 한계 (fold4/133일 원인 규명, 26.07.14 루프11)

4-fold [0.512, 0.351, 0.527, **0.842**] — fold4(cutoff=130, val 130~133일)만 유독 나쁨.
상관계수(r=0.976)·진폭(기울기=0.975)은 오히려 다른 fold보다 좋은데 **편향(+0.667,
체계적 과대예측)** 하나가 RMSE를 지배함.

**원인**: 검증구간 중 133일이 문제. 133일과 똑같은 날씨 템플릿(외기온도 하락 패턴이
3시간 단위로 소수점까지 완전 동일)이 훈련구간에 이미 3번(113/119/125일) 있었음 —
그런데 실제 soil_temp 하락폭이 113/119/125일은 2.37~2.79도로 서로 비슷했던 반면
**133일만 3.83도**로 이례적으로 튐(이전 3번 자체 편차 0.43도의 2배 넘는 이탈).

**farmer 시나리오(액추에이터 비교로 검증)**: 133일만 정오 보온커튼 최대개방률이 63%
(다른 3일은 100%), 순환팬은 하루종일 70~100% 가동(다른 3일은 대부분 꺼짐). 추정:
온도가 예상보다 빨리 떨어지자 순환팬을 더 돌리고 커튼을 덜 열어 대응했지만, 그 과정에서
낮 동안의 자연 태양열 획득 기회를 놓쳐 결과적으로 하루 평균이 더 떨어짐 — 반응 자체가
133일 단 1회뿐이라 통계적으로 학습 불가능한 표본.

**시도 30여개 전부 실패**(단기추세/한파피처/조건부·최근일 가중치/액추에이터 이례성
피처/대안모델 RF·ExtraTrees·HistGB·KNN/블렌드/path_smooth/huber·fair loss) — 전부
악화되거나 "한쪽 fold 개선 시 다른 fold 악화"의 재분배일 뿐 순이익 없음. **결론**:
soil_ec의 관리 이벤트 문제와 동일한 성격 — 액추에이터 값은 관측되지만(1) 반응이 감지되는
시점(9시 이후)엔 이미 하루 손실 상당분이 누적된 뒤이고(2) 표본이 1개뿐이라 트리가 일반
규칙으로 학습할 근거가 없음. train.py 변경 없음, 4-fold mean 0.5583 유지. 상세:
FEEDBACK.md 루프11.
